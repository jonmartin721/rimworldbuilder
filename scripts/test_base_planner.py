...
def visualize_base_plan(
    planner: BasePlanner, offset_x: int, offset_y: int, bridge_positions: list
):
    ...
    for i, room in enumerate(planner.placed_rooms):
        ...
        # Draw room rectangle
        draw2.rectangle(
            [x1 * scale, y1 * scale, (x2 + 1) * scale - 1, (y2 + 1) * scale - 1],
            fill=None,
            outline=color,
            width=2,
        )
        ...
        if font:
            draw2.text(
                (center_x, center_y), room_name, fill=color, font=font, anchor="mm"
            )
