...
class InteractiveVisualizer:
    ...
    def _organize_layers(self):
        ...
        sorted_layers = sorted(
            layers.keys(), key=lambda x: len(layers[x]), reverse=True
        )
    ...
    def setup_plot(self):
        ...
        self.ax_main.set_title(
            f"RimWorld Base Layout - {len(self.parse_result.buildings)} buildings"
        )
        ...
        # Position checkboxes
        np.linspace(0.9, 0.1, len(labels))
        self.check = CheckButtons(self.ax_control, labels, [True] * len(labels))
    ...
    def draw_layers(self):
        ...
        for i, layer in enumerate(self.layers):
            if self.layer_visibility[layer]:
                ...
                marker_size = (
                    3 if layer in [BuildingType.CONDUIT, BuildingType.FLOOR] else 5
                )
    ...
    def add_legend(self):
        ...
        for layer in self.layers[:10]:  # Show top 10 types
            if self.layer_visibility[layer]:
                color = self.get_layer_color(layer)
                count = len(self.layer_data[layer])
                element = patches.Patch(color=color, label=f"{layer.name} ({count})")
                legend_elements.append(element)

        if legend_elements:
            self.ax_main.legend(
                handles=legend_elements, loc="upper right", fontsize=8, framealpha=0.8
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Interactive RimWorld base visualization"
    )
    parser.add_argument(
        "save_file",
        nargs="?",
