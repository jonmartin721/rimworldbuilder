...
def find_buildability():
                ...
                if child.text:
                    try:
                        ...
                        if len(decompressed) == 275 * 275 * 2:
                            ...
                            print(f"    First 10 values: {[f'0x{v:04X}' for v in sample]}")
                            
                            # Check unique values
                            all_values = struct.unpack(f'<{len(decompressed)//2}H', decompressed)
                            unique = set(all_values)
                            print(f"    Unique values: {len(unique)}")
                            if len(unique) < 20:
                                print(f"    Values: {[f'0x{v:04X}' for v in sorted(unique)]}")
    ...
    for elem in first_map.iter():
        ...
        if any(word in tag_lower for word in ['build', 'afford', 'foundation', 'artificial']):
