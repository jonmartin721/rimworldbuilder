        ...
        for i, game_map in enumerate(game_state.maps):
            ...
            if game_map.zones:
                ...
                for zone in game_map.zones:
                    zone_type = (
                        zone.zone_type.split("_")[-1]
                        if "_" in zone.zone_type
                        else zone.zone_type
                    )
            ...
            print(
                f"      Buildable: {buildable_count} ({buildable_count / max(total_tiles, 1) * 100:.1f}%)"
            )
            print(
                f"      Walkable: {walkable_count} ({walkable_count / max(total_tiles, 1) * 100:.1f}%)"
            )

            print("\n      Top Terrain Types:")
            for terrain_type, count in sorted(
                terrain_types.items(), key=lambda x: x[1], reverse=True
            )[:5]:
...
if __name__ == "__main__":
    ...
    if save_file.exists():
        ...
        print(
            "Please ensure your RimWorld save file is placed in the data/saves directory"
        )
