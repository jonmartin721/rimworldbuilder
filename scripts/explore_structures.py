...
def explore_structures(save_file):
    ...
    for def_name, count in def_counts.items():
        ...
        elif 'bed' in def_lower or 'table' in def_lower or 'chair' in def_lower or 'dresser' in def_lower:
            furniture[def_name] = count
        elif 'bench' in def_lower or 'stove' in def_lower or 'brewery' in def_lower or 'smith' in def_lower:
            production[def_name] = count
        elif 'solar' in def_lower or 'battery' in def_lower or 'power' in def_lower or 'generator' in def_lower:
    ...
    if bridges:
        ...
        for name, count in sorted(bridges.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"  {name}: {count}")
    
    if floors:
        print(f"\nFLOORS/TILES ({sum(floors.values())} total):")
        for name, count in sorted(floors.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"  {name}: {count}")
