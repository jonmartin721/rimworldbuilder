...
def visualize_base_layout(save_file_path: Path, focus_area: tuple = None):
    ...
    for building in game_map.buildings:
        if (
            x_start <= building.position.x < x_start + width
            and y_start <= building.position.y < y_start + height
        ):
            x_idx = building.position.x - x_start
            y_idx = building.position.y - y_start

            color = building_colors.get(
                building.building_type or BuildingType.MISC, [0.7, 0.7, 0.7]
            )
    ...
    for zone in game_map.zones:
        zone_type = (
            zone.zone_type.split("_")[-1] if "_" in zone.zone_type else zone.zone_type
        )
        color = zone_colors.get(zone_type, [0.5, 0.5, 0.8])

        for cell in zone.cells:
            if (
                x_start <= cell.x < x_start + width
                and y_start <= cell.y < y_start + height
            ):
    ...
    for colonist in colonists:
        if (
            x_start <= colonist.position.x < x_start + width
            and y_start <= colonist.position.y < y_start + height
        ):
    ...
    total_walls = len(
        [b for b in game_map.buildings if b.building_type == BuildingType.WALL]
    )
    total_doors = len(
        [b for b in game_map.buildings if b.building_type == BuildingType.DOOR]
    )
    total_production = len(
