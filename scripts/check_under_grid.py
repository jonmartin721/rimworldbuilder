    ...
    if things is not None:
        ...
        if bridge_positions:
            ...
            for tid, count in sorted(
                under_at_bridges.items(), key=lambda x: x[1], reverse=True
            ):
                print(f"  0x{tid:04X}: {count} positions")

                # If this is not 0xFF89, it might be the completed bridge ID
                if tid != 0xFF89:
                    print("    ^ This could be completed HeavyBridge terrain!")
        ...
        if completed_bridges:
            ...
            for x, y in completed_bridges[:5]:
                if 0 <= x < 275 and 0 <= y < 275:
                    ...
                    print(
                        f"  At ({x},{y}): top=0x{top_tid:04X}, under=0x{under_tid:04X}"
                    )
        else:
            print(
                "No completed HeavyBridge things found (they might be in terrain only)"
            )
