...
def quick_visualize(save_file_path: Path):
    ...
    for zone in game_map.zones:
        zone_x = [cell.x for cell in zone.cells if x_min <= cell.x <= x_max and y_min <= cell.y <= y_max]
        zone_y = [cell.y for cell in zone.cells if x_min <= cell.x <= x_max and y_min <= cell.y <= y_max]
    ...
    ax.set_title(f'RimWorld Base Layout - {len(game_map.buildings)} buildings, {len(game_map.get_colonists())} colonists')
