...
def test_complete_system():
    ...
    if save_path.exists():
        ...
        if result.maps:
            ...
            if first_map.terrain:
                unique_terrains = len(
                    set(t.def_name for t in first_map.terrain.values())
                )
    ...
    if alpha_path.exists():
        ...
        print(
            f"   [OK] Found: {len(beds)} beds, {len(tables)} tables, {len(doors)} doors"
        )
    else:
    ...
    if alpha_path.exists():
        ...
        print(
            f"   [OK] Learned patterns from {len(analyzer.patterns['room_counts'])} prefabs"
        )
    ...
    for test_input in test_inputs:
        ...
        print(
            f"     -> {requirements.num_colonists} colonists, {requirements.style} style"
        )
    ...
    generator = ImprovedWFCGenerator(
        30, 30, patterns_file if patterns_file.exists() else None
    )

    # Generate a small base
    grid = generator.generate_with_templates(
        num_bedrooms=4, num_workrooms=2, include_kitchen=True, include_storage=True
    )
    ...
    user_request = (
        "Create an efficient base for 5 colonists with kitchen, storage, and workshop"
    )
    ...
    print("   [OK] Saved visualization to test_complete_system.png")
