    ...
    if terrain_elem is not None:
        ...
        if foundation_elem is not None and foundation_elem.text:
            ...
            if len(positions_1a47) > 0:
                ...
                print(
                    f"  Terrain IDs under 0x1A47: {[f'0x{t:04X}' for t in sorted(terrain_under_1a47)]}"
                )
                ...
                print(
                    f"  Spatial pattern - X variance: {x_var:.1f}, Y variance: {y_var:.1f}"
                )
            ...
            if len(positions_8c7d) > 0:
                ...
                print(
                    f"  Terrain IDs under 0x8C7D: {[f'0x{t:04X}' for t in sorted(terrain_under_8c7d)]}"
                )
            ...
            if things is not None:
                ...
                if frame_positions:
                    print(
                        f"Checking {len(frame_positions)} Frame_HeavyBridge positions..."
                    )
                    ...
                    for x, y in frame_positions:
                        if 0 <= x < 275 and 0 <= y < 275:
                            ...
                            foundation_at_frames[fval] = (
                                foundation_at_frames.get(fval, 0) + 1
                            )
            ...
            if len(positions_1a47) > 0:
                ...
                for i in range(min(5, len(positions_1a47))):
                    ...
                    print(
                        f"  Position ({x:3}, {y:3}): terrain=0x{tid:04X} {terrain_name}"
                    )
