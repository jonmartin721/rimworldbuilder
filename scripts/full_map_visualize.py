...
def full_map_visualize(save_file_path: Path):
    ...
    for colonist in game_map.get_colonists():
        ...
        if scale > 1:
            draw.ellipse(
                [x - 1, y - 1, x + 2, y + 2], fill=(255, 0, 0), outline=(255, 255, 0)
            )
        else:
    ...
    if x_coords and y_coords:
        ...
        detail_img = Image.new(
            "RGB",
            (detail_size * detail_scale, detail_size * detail_scale),
            color=(30, 30, 30),
        )
        ...
        for i in range(0, detail_size + 1, 10):
            x = i * detail_scale
            detail_draw.line(
                [(x, 0), (x, detail_size * detail_scale)], fill=(50, 50, 50), width=1
            )
            detail_draw.line(
                [(0, x), (detail_size * detail_scale, x)], fill=(50, 50, 50), width=1
            )
                ...
                if x_min <= cell.x < x_max and y_min <= cell.y < y_max:
                    ...
                    if "Growing" in zone_type:
                        detail_draw.rectangle(
                            [x, y, x + detail_scale - 1, y + detail_scale - 1],
                            fill=(20, 60, 20),
                            outline=None,
                        )
                    elif "Stockpile" in zone_type:
        ...
        for colonist in game_map.get_colonists():
            if (
                ...
                y = (
                    detail_size - 1 - (colonist.position.y - y_min)
                ) * detail_scale + detail_scale // 2
        ...
        print(
            f"Saved detailed view as base_detail_view.png ({building_count} buildings in view)"
        )
