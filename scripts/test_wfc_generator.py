...
def test_wfc_on_bridges(save_file_path: Path):
    ...
    if buildable_positions:
        ...
        entrance_candidates = [
            (x, y) for x, y in buildable_positions if x == entrance_x
        ]
...
def visualize_wfc_result(
    generator: WFCGenerator, offset_x: int, offset_y: int, bridge_positions: set
):
        ...
        for x in range(generator.width):
            ...
            if tile.collapsed and tile.final_type:
                ...
                if tile.final_type == TileType.WALL:
                    ...
                    margin = scale // 4
                    draw.rectangle(
                        [px + margin, py, px + scale - margin - 1, py + scale - 1],
                        fill=color,
                        outline=(0, 0, 0),
                    )
    ...
    print("Saved visualization as wfc_generated_base.png")
