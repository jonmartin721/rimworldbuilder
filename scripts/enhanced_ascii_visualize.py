...
def enhanced_ascii_visualize(save_file_path: Path, focus_on_bridges=False):
    ...
    print(f"Including {len([b for b in game_map.buildings if b.building_type == BuildingType.BRIDGE])} bridges")
    ...
    if focus_on_bridges:
        bridge_positions = [(b.position.x, b.position.y) for b in game_map.buildings 
    ...
    sorted_buildings = sorted(game_map.buildings, 
                            key=lambda b: priority.get(b.building_type, 0))
    ...
    with open('base_layout_enhanced_ascii.txt', 'w') as f:
        f.write(f"RimWorld Base Layout - Enhanced View\n")
        f.write(f"Area: ({x_min}, {y_min}) to ({x_max}, {y_max})\n")
        f.write(f"Bridges: {len([b for b in game_map.buildings if b.building_type == BuildingType.BRIDGE])} total\n")
        f.write("=" * (width + 2) + "\n")
        for row in grid:
            f.write('|' + ''.join(row) + '|\n')
        f.write("=" * (width + 2) + "\n")
