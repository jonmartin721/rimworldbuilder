...
def pil_visualize(save_file_path: Path, scale: int = 4):
    ...
    for building in game_map.buildings:
        if (
            ...
            if building.building_type == BuildingType.WALL:
                ...
                draw.rectangle(
                    [x, y, x + scale - 1, y + scale - 1], fill=color, outline=None
                )
    ...
    for building_type, color in colors.items():
        if building_type is not None:
            legend_draw.rectangle(
                [10, y_offset, 30, y_offset + 15], fill=color, outline=(0, 0, 0)
            )
            legend_draw.text(
                (40, y_offset),
                str(building_type.value if building_type else "Unknown"),
                fill=(0, 0, 0),
            )
            y_offset += 20
    ...
    print("\nBase Statistics:")
    print(
        f"  Total buildings in view: {len([b for b in game_map.buildings if x_min <= b.position.x < x_max and y_min <= b.position.y < y_max])}"
    )
    print(
