...
def layered_visualize(save_file_path: Path, layers_to_show=None):
    ...
    if layers_to_show is None:
        layers_to_show = ['bridges', 'floors', 'walls', 'doors', 'furniture', 
                         'production', 'power', 'zones', 'colonists']
            ...
            for cell in zone.cells:
                if x_min <= cell.x < x_max and y_min <= cell.y < y_max:
                    ...
                    if 'Growing' in zone_type:
                        zone_draw.rectangle([x, y, x + scale - 1, y + scale - 1], 
                                          fill=(20, 60, 20, 100))
    ...
    if 'bridges' in layers_to_show:
        ...
        for building in bridges:
            if x_min <= building.position.x < x_max and y_min <= building.position.y < y_max:
                ...
                bridge_draw.rectangle([x, y, x + scale - 1, y + scale - 1], 
                                     fill=(101, 67, 33, 255), outline=(60, 40, 20, 255))
    ...
    if 'floors' in layers_to_show:
        ...
        floors = [b for b in game_map.buildings if b.building_type == BuildingType.FLOOR]
        for building in floors:
            if x_min <= building.position.x < x_max and y_min <= building.position.y < y_max:
                x = (building.position.x - x_min) * scale
                y = (height - 1 - (building.position.y - y_min)) * scale
                
                floor_draw.rectangle([x, y, x + scale - 1, y + scale - 1], 
                                   fill=(80, 80, 80, 150))
    ...
    if 'conduits' in layers_to_show:
        ...
        conduits = [b for b in game_map.buildings if b.building_type == BuildingType.CONDUIT]
        ...
        for building in conduits:
            if x_min <= building.position.x < x_max and y_min <= building.position.y < y_max:
                ...
                conduit_draw.ellipse([x - 2, y - 2, x + 2, y + 2], 
                                    fill=(255, 255, 0, 150))
    ...
    if 'walls' in layers_to_show:
        ...
        for building in walls:
            if x_min <= building.position.x < x_max and y_min <= building.position.y < y_max:
                ...
                wall_draw.rectangle([x, y, x + scale - 1, y + scale - 1], 
    ...
    if 'doors' in layers_to_show:
        ...
        for building in doors:
            if x_min <= building.position.x < x_max and y_min <= building.position.y < y_max:
                ...
                margin = scale // 4
                door_draw.rectangle([x + margin, y, x + scale - margin - 1, y + scale - 1], 
                                  fill=(139, 69, 19, 255), outline=(80, 40, 10, 255))
    ...
    if 'furniture' in layers_to_show:
        ...
        furniture = [b for b in game_map.buildings if b.building_type == BuildingType.FURNITURE]
        for building in furniture:
            if x_min <= building.position.x < x_max and y_min <= building.position.y < y_max:
                ...
                margin = scale // 4
                furniture_draw.ellipse([x + margin, y + margin, 
                                      x + scale - margin - 1, y + scale - margin - 1], 
                                     fill=(70, 130, 180, 255), outline=(40, 80, 120, 255))
        layers['furniture'] = furniture_layer
    
    # Production layer
    if 'production' in layers_to_show:
        production_layer = Image.new('RGBA', (img_width, img_height), color=(0, 0, 0, 0))
        production_draw = ImageDraw.Draw(production_layer)
        
        production = [b for b in game_map.buildings if b.building_type == BuildingType.PRODUCTION]
        for building in production:
            if x_min <= building.position.x < x_max and y_min <= building.position.y < y_max:
                x = (building.position.x - x_min) * scale
                y = (height - 1 - (building.position.y - y_min)) * scale
                
                production_draw.rectangle([x + 1, y + 1, x + scale - 2, y + scale - 2], 
                                        fill=(255, 215, 0, 255), outline=(180, 150, 0, 255))
    ...
    if 'power' in layers_to_show:
        ...
        for building in power:
            if x_min <= building.position.x < x_max and y_min <= building.position.y < y_max:
                ...
                power_draw.rectangle([x + 1, y + 1, x + scale - 2, y + scale - 2], 
                                   fill=(255, 140, 0, 255), outline=(180, 100, 0, 255))
    ...
    if 'colonists' in layers_to_show:
        ...
        for colonist in game_map.get_colonists():
            if x_min <= colonist.position.x < x_max and y_min <= colonist.position.y < y_max:
                ...
                colonist_draw.ellipse([x - scale//2, y - scale//2, 
                                     x + scale//2, y + scale//2], 
                                    fill=(255, 255, 255, 255), outline=(255, 0, 0, 255), width=2)
    ...
    for layer_name, layer in layers.items():
        if layer_name in layers_to_show:
            layer_img = Image.new('RGBA', (img_width, img_height), color=(30, 30, 30, 255))
    ...
    # Print statistics
    print("\nLayer Statistics:")
    print(f"  Bridges: {len([b for b in game_map.buildings if b.building_type == BuildingType.BRIDGE])}")
    print(f"  Floors: {len([b for b in game_map.buildings if b.building_type == BuildingType.FLOOR])}")
    ...
    print(f"  Conduits: {len([b for b in game_map.buildings if b.building_type == BuildingType.CONDUIT])}")
