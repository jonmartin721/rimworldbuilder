...
from src.models.game_entities import Building, BuildingType, Position, Colonist
...
class TestSaveParser:
    ...
    def test_is_building(self, parser):
        ...
        assert parser._is_building("Wall_WoodPlank") == True
        assert parser._is_building("Door_Steel") == True
        assert parser._is_building("Bed_Wood") == True
        assert parser._is_building("Tree_Oak") == False
        assert parser._is_building("Human") == False
    ...
    def test_parse_building_xml(self, parser):
        ...
        building = parser._parse_building(thing, "wall_123", "Wall_WoodPlank", Position(10, 20, 0))
    ...
    def test_parse_frame_building(self, parser):
        ...
        """
        thing = ET.fromstring(xml_str)
        building = parser._parse_building(thing, "frame_456", "Frame_HeavyBridge", Position(50, 60, 0))
        
        assert building.def_name == "Frame_HeavyBridge"
        assert building.building_type == BuildingType.BRIDGE
        assert building.is_frame == True
    ...
    @patch('builtins.open', new_callable=mock_open, read_data='<?xml version="1.0"?><savegame></savegame>')
