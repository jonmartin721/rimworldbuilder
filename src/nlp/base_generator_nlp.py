...
class BaseGeneratorNLP:
    ...
    ROOM_KEYWORDS = {
        ...
        'workshop': ['workshop', 'production', 'crafting', 'manufacturing', 'workbench'],
    ...
    DEFENSE_KEYWORDS = {
        'high': ['heavily defended', 'maximum defense', 'fortress', 'highly secure', 'impenetrable'],
    ...
    def parse_request(self, user_input: str) -> BaseRequirements:
        ...
        workshop_match = re.search(r'(\d+)\s*(?:workshop|workroom|production)', input_lower)
        if workshop_match:
            requirements.num_workrooms = int(workshop_match.group(1))
        
        # Parse room inclusions
        requirements.include_kitchen = self._check_keywords(input_lower, self.ROOM_KEYWORDS['kitchen'])
        requirements.include_dining = self._check_keywords(input_lower, self.ROOM_KEYWORDS['dining'])
        requirements.include_rec = self._check_keywords(input_lower, self.ROOM_KEYWORDS['recreation'])
        requirements.include_storage = self._check_keywords(input_lower, self.ROOM_KEYWORDS['storage'])
        requirements.include_medical = self._check_keywords(input_lower, self.ROOM_KEYWORDS['medical'])
        requirements.include_research = self._check_keywords(input_lower, self.ROOM_KEYWORDS['research'])
        requirements.include_prison = self._check_keywords(input_lower, self.ROOM_KEYWORDS['prison'])
        ...
        requirements.special_requirements = self._extract_special_requirements(input_lower)
    ...
    def _apply_style_presets(self, requirements: BaseRequirements):
        ...
        if requirements.style == 'defensive':
            ...
            requirements.num_bedrooms = max(requirements.num_bedrooms, requirements.num_colonists + 2)
            ...
            requirements.num_bedrooms = min(requirements.num_bedrooms, requirements.num_colonists)
            requirements.num_workrooms = max(1, requirements.num_workrooms - 1)
    
    def generate_base(self, user_input: str, 
                     width: int = 50, height: int = 50) -> Tuple[np.ndarray, str]:
        """
    ...
    def _generate_description(self, requirements: BaseRequirements, 
                             generator: ImprovedWFCGenerator) -> str:
        """Generate description of what was created"""
        lines = []
        lines.append(f"Generated {requirements.style} base for {requirements.num_colonists} colonists:")
    ...
    def interpret_feedback(self, feedback: str, current_requirements: BaseRequirements) -> BaseRequirements:
        """
        ...
        if 'add' in feedback_lower or 'need' in feedback_lower or 'want' in feedback_lower:
        ...
        if 'remove' in feedback_lower or "don't need" in feedback_lower or 'no' in feedback_lower:
            for room_type, keywords in self.ROOM_KEYWORDS.items():
                if self._check_keywords(feedback_lower, keywords):
