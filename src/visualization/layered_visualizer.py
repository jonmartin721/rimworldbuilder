...
class LayeredVisualizer:
    ...
    def visualize(self, grid: np.ndarray, 
        ...
        if title:
            try:
                ...
                font = None
            draw.text((total_width // 2, 20), title, 
        ...
        for y in range(height):
            for x in range(width):
                ...
                tile_value = int(grid[display_y, x])
                
                # Find which layer this tile belongs to
                color = base_color
                for layer_id, layer_data in self.LAYERS.items():
                    if tile_value in layer_data["tiles"] and self.enabled_layers[layer_id]:
        ...
        if show_grid and self.scale > 4:
            ...
            for x in range(width + 1):
                px = x * self.scale
                draw.line([(px, title_height), (px, height * self.scale + title_height)], 
                         fill=grid_color, width=1)
            
            # Horizontal lines
            for y in range(height + 1):
                py = y * self.scale + title_height
                draw.line([(0, py), (width * self.scale, py)], 
                         fill=grid_color, width=1)
    ...
    def _draw_layer_legend(self, draw: ImageDraw.Draw, x_start: int, y_start: int):
        ...
        # Title
        draw.text((legend_x + legend_width // 2, legend_y + 15), "Layers", 
                 fill=(255, 255, 255), font=font, anchor="mm")
        ...
        for layer_id, layer_data in self.LAYERS.items():
            ...
            check_x = legend_x + 10
            check_size = 12
            draw.rectangle([check_x, y, check_x + check_size, y + check_size],
                          fill=(100, 200, 100) if enabled else (60, 60, 60),
                          outline=(150, 150, 150))
            
            if enabled:
                # Draw checkmark
                draw.line([(check_x + 2, y + 6), (check_x + 5, y + 9)], 
                         fill=(255, 255, 255), width=2)
                draw.line([(check_x + 5, y + 9), (check_x + 10, y + 3)], 
                         fill=(255, 255, 255), width=2)
            
            # Color sample
            color_x = check_x + check_size + 8
            draw.rectangle([color_x, y + 2, color_x + 20, y + check_size - 2],
                          fill=layer_data["color"] if enabled else (60, 60, 60))
            
            # Layer name
            text_x = color_x + 25
            text_color = (255, 255, 255) if enabled else (128, 128, 128)
            draw.text((text_x, y + check_size // 2), layer_data["name"],
                     fill=text_color, font=small_font, anchor="lm")
    ...
    def create_composite(self, grid: np.ndarray, 
                        layers_to_show: Optional[List[str]] = None) -> Image.Image:
