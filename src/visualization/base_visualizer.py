...
class BaseVisualizer:
    ...
    def visualize(self, 
        ...
        if self.show_grid and self.scale > 4:
            ...
            for x in range(width + 1):
                px = x * self.scale
                draw.line([(px, y_offset), (px, height * self.scale + y_offset)], 
    ...
    def _draw_legend(self, draw: ImageDraw.Draw, x_start: int, y_start: int, 
                    tile_types: set):
        ...
        # Legend background
        draw.rectangle([x_start, y_start, x_start + 150, y_start + len(tile_types) * 20 + 20],
                      fill=(30, 30, 30))
        
        # Title
        draw.text((x_start + 10, y_start + 5), "Tile Types", 
        ...
        for tile_type in sorted(tile_types):
            if tile_type in self.colors:
                ...
                # Color box
                draw.rectangle([x_start + 10, y, x_start + 25, y + 12], 
                             fill=color, outline=(255, 255, 255))
                
                # Label
                draw.text((x_start + 30, y + 2), name, 
    ...
    def create_comparison(self, 
                         ...
                         filename: str = "comparison.png") -> Image.Image:
        """
        ...
        for label, grid in grids.items():
            # Create temp image
            temp_img = self.visualize(grid, f"temp_{label}.png", title=label, 
                                     show_legend=False)
