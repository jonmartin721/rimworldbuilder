...
class HybridPrefabGenerator(ImprovedWFCGenerator):
    ...
    def _load_prefab_library(self, alpha_prefabs_path: Path):
        ...
        print(
            f"Loaded {len(all_layouts)} prefabs in {len(self.prefab_library)} categories"
        )
    ...
    def _categorize_prefab(self, def_name: str) -> str:
        ...
        if "bedroom" in name_lower:
            ...
            "kitchen" in name_lower or "eating" in name_lower or "dining" in name_lower
        ):
            return "kitchen"
    ...
    def _place_prefab(self, prefab: AlphaPrefabLayout, category: str) -> bool:
        ...
        # Track placement
        self.placed_prefabs.append(
            PlacedPrefab(
                layout=prefab, position=position, rotation=0, category=category
            )
        )
    ...
    def _choose_tile_type_near_prefabs(self, x: int, y: int) -> TileType:
        ...
        if adjacent_types:
            if TileType.CORRIDOR in adjacent_types:
                ...
                t in [TileType.BEDROOM, TileType.KITCHEN, TileType.STORAGE]
    ...
    def _create_corridor_between_prefabs(
        self, prefab1: PlacedPrefab, prefab2: PlacedPrefab
    ):
