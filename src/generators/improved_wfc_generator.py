...
class ImprovedWFCGenerator(WFCGenerator):
    ...
    def generate_with_templates(self, buildable_mask: Optional[np.ndarray] = None,
                               num_bedrooms: int = 4,
                               num_workrooms: int = 2,
                               include_kitchen: bool = True,
                               include_rec: bool = True,
                               include_storage: bool = True) -> np.ndarray:
        """
        ...
        room_plan = self._plan_room_layout(
            num_bedrooms, num_workrooms, 
            include_kitchen, include_rec, include_storage
        )
        ...
        # Run WFC to smooth transitions
        self.collapse()
        
        return self.grid
    
    def _plan_room_layout(self, num_bedrooms: int, num_workrooms: int,
                         include_kitchen: bool, include_rec: bool,
                         include_storage: bool) -> List[Dict]:
    ...
    def _find_room_placement(self, width: int, height: int, 
                            is_central: bool) -> Optional[Tuple[int, int]]:
    ...
    def _is_valid_room_position(self, x: int, y: int, 
        ...
        for dy in range(-1, height + 1):
            for dx in range(-1, width + 1):
                px, py = x + dx, y + dy
                if 0 <= px < self.width and 0 <= py < self.height:
                    if self.collapsed[py, px] and self.grid[py, px] != TileType.EMPTY.value:
                        return False
            ...
            for x in range(self.width):
                if not self.collapsed[y, x]:
                    ...
                    if neighbors:
                        ...
                        neighbor_types = [self.grid[ny, nx] for nx, ny in neighbors 
                                        if self.collapsed[ny, nx]]
                        if any(t > TileType.WALL.value for t in neighbor_types):
