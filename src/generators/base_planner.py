...
class BasePlanner:
    ...
    def __init__(self, width: int, height: int, 
        ...
        self.width = width
        self.height = height
        self.buildable_positions = buildable_positions or {(x, y) for x in range(width) for y in range(height)}
        self.random = random.Random(seed)
        
        self.room_library = RoomTemplateLibrary()
        self.placed_rooms: List[PlacedRoom] = []
        self.grid = np.zeros((height, width), dtype=int)  # 0=empty, 1=wall, 2=door, 3=room interior
    ...
    def plan_base(self, required_rooms: Optional[List[RoomType]] = None) -> bool:
        ...
        templates_to_place.sort(key=lambda t: (t.priority, t.width * t.height), reverse=True)
                ...
                if valid_positions:
                    ...
                    if self.placed_rooms:
                        valid_positions.sort(
                            key=lambda p: min(self._distance_to_room(p, room) 
    ...
    def _find_valid_positions(self, template: RoomTemplate) -> List[Tuple[int, int]]:
        ...
        for y in range(self.height - template.height + 1):
            for x in range(self.width - template.width + 1):
                ...
                if np.sum(area) >= template.width * template.height * 0.8:  # 80% coverage required
                    valid.append((x, y))
    ...
    def _can_place_room_at(self, template: RoomTemplate, x: int, y: int) -> bool:
        ...
        # Check bounds
        if x < 0 or y < 0 or x + template.width > self.width or y + template.height > self.height:
            return False
