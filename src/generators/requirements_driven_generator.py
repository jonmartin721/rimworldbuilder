...
from src.generators.enhanced_hybrid_generator import EnhancedHybridGenerator, PrefabUsageMode, PrefabFragment
...
class RequirementsDrivenGenerator(EnhancedHybridGenerator):
    ...
    def generate_from_requirements(self, 
                                  ...
                                  design_plan: Optional[BaseDesignPlan] = None) -> np.ndarray:
        """
        ...
        prefab_matches.sort(key=lambda m: (m.room_spec.priority, 
                                          -m.prefab.width * m.prefab.height))
        ...
        for i, match in enumerate(prefab_matches):
            ...
            if self._try_place_prefab_smart(match, placed_prefabs):
                ...
                logger.detail(f"✓ Placed {match.room_spec.room_type}: {match.prefab.def_name} (score: {match.score:.2f})")
        ...
        # Summary
        logger.success(f"Placed {len(placed_prefabs)}/{len(prefab_matches)} prefabs successfully")
        logger.finish()
    ...
    def _score_prefab_for_spec(self, prefab: AlphaPrefabLayout, spec: RoomSpec) -> Tuple[float, List[str]]:
        ...
        if area_diff < 0.2:
            score += 5.0
            reasons.append(f"Excellent size match ({actual_area} vs {target_area} tiles)")
        elif area_diff < 0.5:
            score += 3.0
        ...
        if prefab.def_name not in [p.prefab.def_name for p in self.placed_prefabs if hasattr(p, 'prefab')]:
            score += 1.0
            reasons.append("Adds variety")
        
        return score, reasons
    
    def _try_place_prefab_smart(self, match: PrefabMatch, placed: List[PrefabMatch]) -> bool:
