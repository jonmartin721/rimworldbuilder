...
from src.generators.hybrid_prefab_generator import HybridPrefabGenerator, PlacedPrefab
...
class EnhancedHybridGenerator(HybridPrefabGenerator):
        ...
        for category, prefabs in self.prefab_library.items():
            for prefab in prefabs[:10]:  # Analyze first 10 of each category
                ...
                for pattern_name, pattern in patterns.items():
                    self.decorative_patterns[f"{prefab.def_name}_{pattern_name}"] = pattern
            ...
            for x in range(1, layout.shape[1] - 1):
                if not visited[y, x] and layout[y, x] != 1:  # Not wall and not visited
                    ...
                    if 4 <= len(room_tiles) <= 50:  # Reasonable room size
                        ...
                        # Include walls
                        room_layout = layout[min_y-1:max_y+2, min_x-1:max_x+2].copy()
                        
                        rooms.append(PrefabFragment(
                            source_prefab=prefab.def_name,
                            fragment_type="room",
                            layout=room_layout,
                            metadata={
                                'size': len(room_tiles),
                                'aspect_ratio': (max_x - min_x) / (max_y - min_y) if max_y != min_y else 1
                            }
                        ))
        
        return rooms
    
    def _extract_decorative_patterns(self, prefab: AlphaPrefabLayout) -> Dict[str, np.ndarray]:
    ...
    def _extract_furniture_arrangements(self, prefab: AlphaPrefabLayout) -> List[PrefabFragment]:
                ...
                for x in range(layout.shape[1]):
                    if furniture_mask[y, x] and not visited[y, x]:
                        ...
                        if 2 <= len(cluster) <= 10:  # Reasonable cluster size
                            ...
                            arrangement_layout = layout[min_y:max_y+1, min_x:max_x+1].copy()
                            
                            arrangements.append(PrefabFragment(
                                source_prefab=prefab.def_name,
    ...
    def generate_enhanced(self,
                         ...
                         decoration_density: float = 0.2) -> np.ndarray:
        """
    ...
    def _is_valid_fragment_position(self, x: int, y: int, width: int, height: int) -> bool:
        ...
        for y in range(self.height // 3, 2 * self.height // 3):
            for x in range(center_x):
                ...
                if mirror_x < self.width:
                    if self.grid[y, x] in [TileType.BEDROOM.value, TileType.STORAGE.value]:
    ...
    def _terrain_to_array(self, terrain_grid: List[List[str]]) -> np.ndarray:
        ...
        terrain_map = {'.': 0, 'WoodPlankFloor': 1, 'StrawMatting': 2, 
                      'CarpetPurpleDeep': 3, 'TileLimestone': 4}
    ...
    def _flood_fill_room(self, layout: np.ndarray, start_x: int, start_y: int, 
    ...
    def _find_furniture_cluster(self, layout: np.ndarray, start_x: int, start_y: int,
                               visited: np.ndarray) -> List[Tuple[int, int]]:
