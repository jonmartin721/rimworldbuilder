...
class ModParser:
    ...
    def _parse_thing_def(self, thing_def: ET.Element) -> Optional[BuildingDef]:
        ...
        try:
            ...
            # Parse passability
            building.passability = self._get_text(thing_def, "passability", "Impassable")
            building.fill_percent = self._get_float(thing_def, "fillPercent", 1.0)
            building.terrain_affordance = self._get_text(thing_def, "terrainAffordanceNeeded", "Light")
    ...
    def _is_building_def(self, building_def: BuildingDef) -> bool:
        ...
        return (building_def.size != (1, 1) or 
                building_def.is_door or 
                building_def.is_bed or
                building_def.is_table or
                building_def.is_chair or
                building_def.is_storage or
                building_def.is_production or
                building_def.is_power or
                "Wall" in building_def.def_name or
                "Door" in building_def.def_name or
                "Conduit" in building_def.def_name)
    
    def _is_door(self, thing_def: ET.Element) -> bool:
        """Check if this is a door"""
        thing_class = self._get_text(thing_def, "thingClass", "")
        return "Door" in thing_class or "Door" in self._get_text(thing_def, "defName", "")
    
    def _is_bed(self, thing_def: ET.Element) -> bool:
        """Check if this is a bed"""
        thing_class = self._get_text(thing_def, "thingClass", "")
        def_name = self._get_text(thing_def, "defName", "")
        return ("Bed" in thing_class or "Bed" in def_name or 
                "Bedroll" in def_name or "SleepingSpot" in def_name)
    ...
    def _is_storage(self, thing_def: ET.Element) -> bool:
        ...
        def_name = self._get_text(thing_def, "defName", "")
        thing_class = self._get_text(thing_def, "thingClass", "")
        return ("Storage" in thing_class or "Shelf" in def_name or 
                "Container" in def_name or "Stockpile" in def_name)
    
    def _is_production(self, thing_def: ET.Element) -> bool:
        """Check if this is a production building"""
        def_name = self._get_text(thing_def, "defName", "")
        return ("Bench" in def_name or "Table_" in def_name or 
                "Stove" in def_name or "Brewery" in def_name or
                "Loom" in def_name or "Smith" in def_name)
    
    def _is_power(self, thing_def: ET.Element) -> bool:
        """Check if this is power-related"""
        def_name = self._get_text(thing_def, "defName", "")
        comp_power = thing_def.find(".//CompProperties_Power")
        return (comp_power is not None or "Power" in def_name or 
                "Battery" in def_name or "Solar" in def_name or
                "Wind" in def_name or "Geothermal" in def_name)
    ...
    def _is_joy(self, thing_def: ET.Element) -> bool:
        ...
        def_name = self._get_text(thing_def, "defName", "")
        return ("Television" in def_name or "GameBoard" in def_name or
                "Telescope" in def_name or "Pool" in def_name)
    ...
    def get_buildings_by_category(self, category: str) -> List[BuildingDef]:
        ...
        return [b for b in self.building_defs.values() if category.lower() in b.category.lower()]
    ...
    def get_buildings_by_type(self, 
                            ...
                            is_power: bool = None,
                            is_joy: bool = None) -> List[BuildingDef]:
