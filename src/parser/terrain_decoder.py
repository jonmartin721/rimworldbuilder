...
class TerrainDecoder:
    ...
    def decode_terrain_grid(self, map_elem: etree.Element) -> np.ndarray | None:
        ...
        try:
            ...
            if len(decompressed) != expected_size:
                logger.warning(
                    f"Decompressed size {len(decompressed)} != expected {expected_size}"
                )
    ...
    def decode_foundation_grid(self, map_elem: etree.Element) -> np.ndarray | None:
        ...
        try:
            ...
            map_width, map_height = self._get_map_size(map_elem)
            num_tiles = len(decompressed) // 2
            foundation_ids = struct.unpack(
                f"<{num_tiles}H", decompressed[: num_tiles * 2]
            )
    ...
    def _analyze_terrain_types(self, grid: np.ndarray):
        ...
        for terrain_id in unique_ids:
            ...
            terrain_name = self.TERRAIN_IDS.get(
                int(terrain_id), f"Unknown_{terrain_id:04X}"
            )
