...
class RimWorldSaveParser:
    ...
    def _extract_game_state(self, root: etree.Element) -> GameState:
        ...
        game_state = GameState(
            ...
            save_name=self._get_text(info, "realWorldDate", "Unknown")
            if info is not None
            else "Unknown",
            seed=self._get_text(info, "seedString", "") if info is not None else "",
            play_time=float(self._get_text(info, "realPlayTimeInteracting", "0"))
            if info is not None
            else 0,
            tick=int(self._get_text(info, "ticksGame", "0")) if info is not None else 0,
            date=self._get_text(info, "dateStringShort", "")
            if info is not None
            else "",
        )
        ...
        if meta is not None:
            ...
            if mod_list is not None:
                game_state.mod_ids = [
                    li.text for li in mod_list.findall("li") if li.text
                ]

            mod_names = meta.find("modNames")
            if mod_names is not None:
                game_state.mod_names = [
                    li.text for li in mod_names.findall("li") if li.text
                ]
    ...
    def _parse_map(self, map_elem: etree.Element) -> Optional[Map]:
        try:
            ...
            game_map = Map(map_id=map_id, size=map_size)
    ...
    def _decode_deflated_terrain(self, deflated_data: str, game_map: Map):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Failed to decompress terrain data with any method"
                        )
                        return
            ...
            logger.info(
                f"Decompressed {len(decompressed_data)} bytes for {width}x{height} map (expected ~{expected_size} values)"
            )
            ...
            if len(decompressed_data) >= expected_size * 2:  # Could be 2 bytes per tile
                ...
                logger.warning(
                    f"Terrain data size mismatch: got {len(decompressed_data)} bytes, expected at least {expected_size}"
                )
    ...
    def _parse_binary_terrain_data(self, binary_data: bytes, game_map: Map):
        ...
        for i in range(0, min(len(binary_data) - 1, width * height * 2), 2):
            ...
            # Map to terrain name
            terrain_def = terrain_map.get(
                terrain_bytes, f"Unknown_{terrain_bytes.hex()}"
            )

            # Create terrain tile with proper walkable/buildable logic
            walkable = (
                terrain_def not in ["Water", "RoughStone"] and "Wall" not in terrain_def
            )
            buildable = (
                terrain_def not in ["Water", "RoughStone", "Ice", "MarshyTerrain"]
                and "Wall" not in terrain_def
            )
    ...
    def _parse_things(self, map_elem: etree.Element, game_map: Map):
        ...
        for thing in things.findall("thing"):
            ...
            if (
                "Building" in thing_class
                or "Frame" in thing_class
                or self._is_building(def_name)
            ):
    ...
    def _parse_pawn(
        ...
        pawn = Pawn(id=thing_id, name=name, position=pos)
    ...
    def _parse_zone(self, zone_elem: etree.Element) -> Optional[Zone]:
        ...
        zone = Zone(id=zone_id, name=zone_label, zone_type=zone_type, cells=[])
    ...
    def _parse_areas(self, map_elem: etree.Element, game_map: Map):
        ...
        if home_area is not None:
            ...
            if inner_grid is not None and inner_grid.text:
                game_map.home_area = self._decode_area_grid(
                    inner_grid.text, game_map.size
                )
        ...
        if len(parts) >= 2:
            try:
                ...
                if len(parts) >= 3:
                    ...
                    y = int(parts[1].strip())
                z = (
                    int(parts[1].strip()) if len(parts) > 2 else 0
                )  # Middle value is elevation
