...
from src.generators.enhanced_hybrid_generator import (
    EnhancedHybridGenerator,
    PrefabUsageMode,
)
...
class RimWorldAssistant:
    ...
    def __init__(self):
        ...
        if not self.alpha_prefabs_path.exists():
            ...
            print(
                "To get full functionality, clone: https://github.com/juanosarg/AlphaPrefabs"
            )
    ...
    def print_menu(self):
        ...
        if self.last_grid is not None:
            print(
                f"Last generation: {self.last_grid.shape[0]}x{self.last_grid.shape[1]} tiles"
            )
    ...
    def analyze_current_base(self):
        ...
        if bridges:
            ...
            print(
                f"  Coverage: {len(bridges)} tiles ({len(bridges) / ((max_x - min_x + 1) * (max_y - min_y + 1)) * 100:.1f}%)"
            )
    ...
    def generate_with_prefabs(self):
        ...
        print(
            "Available: bedroom, kitchen, storage, workshop, medical, recreation, power"
        )
        ...
        grid = generator.generate_with_prefab_anchors(
            prefab_categories=category_list, num_prefabs=num_prefabs, fill_with_wfc=True
        )
        self.last_grid = grid
    ...
    def generate_hybrid(self):
        ...
        grid = generator.generate_enhanced(
            usage_modes=modes, prefab_density=density, decoration_density=0.2
        )
        self.last_grid = grid
    ...
    def smart_generate(self):
        ...
        if not self.alpha_prefabs_path:
            ...
            print(
                "Please clone: https://github.com/juanosarg/AlphaPrefabs into data/AlphaPrefabs"
            )
            return
        ...
        with spinner("Parsing natural language"):
            if not self.nlp:
                self.nlp = BaseGeneratorNLP(
                    str(self.save_path) if self.save_path else None
                )
        ...
        use_ai = (
            input(
                f"\n{ROBOT} Use Claude AI for detailed planning? (y/n, default n): "
            ).lower()
            == "y"
        )
        ...
        grid = generator.generate_from_requirements(
            requirements=requirements, design_plan=plan
        )
        self.last_grid = grid
    ...
    def ai_design_base(self):
        ...
        priority_list = (
            [p.strip() for p in priorities.split(",")] if priorities else ["balanced"]
        )

        special = input(
            "Special requirements (e.g., killbox,hospital,throne_room): "
        ).strip()
    ...
    def save_visualization(self, grid, filename):
        ...
        with spinner(f"Saving visualization to {filename}"):
            ...
            visualizer.visualize(
                grid, filename, title="Generated RimWorld Base", show_legend=True
            )
...
def main():
    ...
    parser.add_argument(
        "--output", help="Output filename", default="generated_base.png"
    )
